...
public class CFG {
	...
	static int startIdx =0, endIdx=0;
	...
	*/
		...
		for (int i = 0; i <= m; i++)
		{
			for (int j = 0; j <= n; j++)
			{
				...
				else if (X[i - 1] == Y[j - 1])
				{
					...
					//bijaya  - commented above line and added to get the string 
	...
	public static void main(String[] args)
	{
        ...
        //  String Y="OewSite:GeeksQuiz.com";
		int m = X.length();
		int n = Y.length();

		System.out.println(LCSubStr(X.toCharArray(),Y.toCharArray(), m,n));
		System.out.println("--"+X.substring(startIdx, endIdx));
	}
}
